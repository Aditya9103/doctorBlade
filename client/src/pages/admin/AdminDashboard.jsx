...
const StatCard = ({ label, value, sub, subColor = 'text-blue-600', icon, iconBg }) => (
  <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm flex flex-col gap-3">
    <div className="flex items-start justify-between">
      <p className="text-xs font-bold text-black-500 uppercase tracking-widest">{label}</p>
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
            ...
            <StatCard
              ...
              icon={<svg className="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>}
            />
            <StatCard
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              ...
              <span className="text-xs text-slate-900 font-semibold">{stats?.totalSubmissions} total</span>
            </div>
            {recent.length === 0 ? (
              <div className="py-16 text-center text-slate-900 text-sm font-medium">No submissions yet.</div>
            ) : (
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {recent.map((s) => (
                      <tr key={s._id} className="hover:bg-slate-50/60 transition-colors">
                        <td className="px-6 py-3.5 font-semibold text-slate-900">{s.fullName}</td>
                        <td className="px-6 py-3.5">
                          <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-bold ${s.type === 'quote' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'}`}>
                            {s.type === 'quote' ? 'Quote' : 'Contact'}
                          </span>
                        </td>
                        <td className="px-6 py-3.5 text-slate-900">{s.email}</td>
                        <td className="px-6 py-3.5 text-slate-900 text-xs">{new Date(s.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                        <td className="px-6 py-3.5">
                          <span className={`inline-flex items-center gap-1.5 text-xs font-semibold ${s.isRead ? 'text-slate-400' : 'text-blue-600'}`}>
                            {!s.isRead && <span className="w-1.5 h-1.5 rounded-full bg-blue-600 inline-block" />}
                            {s.isRead ? 'Read' : 'New'}
                          </span>
                        </td>
                      </tr>
